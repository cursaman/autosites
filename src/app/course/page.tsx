...
import decisionStyles from "./decision-guide.module.css";
...
export default function CoursePage() {
  const { hero, promise, instructor, progress, records, audience, decisionGuide, apiWorkshop, curriculum, outcomes, completion, nextCohort, fee, venue, preparation, recruitment, applicationFlow } = courseContent;
  ...
  return (
    <div className={styles.page}>
      ...
      <main id="course-main" className={styles.main} tabIndex={-1}>
        ...
        <section id="course-comparison" className={`${styles.section} ${styles.container}`} aria-labelledby="course-comparison-title"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>{decisionGuide.eyebrow}</p><h2 id="course-comparison-title">{decisionGuide.title}</h2></div><p>{decisionGuide.description}</p></div><div className={decisionStyles.tableWrap}><table><thead><tr>{decisionGuide.columns.map((column) => <th key={column} scope="col">{column}</th>)}</tr></thead><tbody>{decisionGuide.rows.map((row) => <tr key={row.label}><th scope="row">{row.label}</th><td>{row.experience}</td><td>{row.course}</td></tr>)}</tbody></table></div><div className={decisionStyles.choiceActions}><Link href={decisionGuide.experienceAction.href}>{decisionGuide.experienceAction.label} →</Link><a href={decisionGuide.courseAction.href}>{decisionGuide.courseAction.label} ↓</a></div></section>
        ...
        <section id="application-flow" className={`${styles.section} ${styles.container}`} aria-labelledby="application-flow-title"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>{applicationFlow.eyebrow}</p><h2 id="application-flow-title">{applicationFlow.title}</h2></div><p>{applicationFlow.description}</p></div><ol className={decisionStyles.flow}>{applicationFlow.steps.map((step) => <li key={step.number}><span>{step.number}</span><div><h3>{step.title}</h3><p>{step.description}</p></div></li>)}</ol><div className={decisionStyles.flowActions}><Link href={applicationFlow.setupAction.href}>{applicationFlow.setupAction.label} →</Link><a href={applicationFlow.inquiryAction.href} target="_blank" rel="noreferrer">{applicationFlow.inquiryAction.label} <span aria-hidden="true">↗</span></a></div></section>
