...
const referenceGroups = [
  { number: "01", title: "AI·홈페이지 학습", description: "AI 기초 용어부터 홈페이지 제작과 데이터베이스 실습까지 이어지는 학습 사이트" },
...
export default function ResourcesPage() {
  ...
  return <div className={styles.page}>
    ...
    <main id="resources-main" tabIndex={-1}>
      ...
      <section id="reference-links" className={`${styles.references} ${styles.container}`} aria-labelledby="reference-title"><div className={styles.heading}><div><p className={styles.eyebrow}>AI TOGETHER RESOURCES</p><h2 id="reference-title">디자인·코딩 참고자료</h2></div><p>AI 기초학습부터 수업용 확장 프로그램, 디자인과 웹 개발 문서까지 필요한 자료를 종류별로 확인하세요.</p></div><ol>{referenceGroups.map((group)=><li key={group.number}><span>{group.number}</span><div><h3>{group.title}</h3><p>{group.description}</p></div></li>)}</ol><a className={styles.referenceAction} href="https://ai-together-mu.vercel.app/resources" target="_blank" rel="noreferrer">AI Together 참고자료 전체보기 <span>↗</span></a></section>
