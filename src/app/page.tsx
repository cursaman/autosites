...
export default function HomePage() {
  ...
  return <div id="top" className={styles.page}>
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      <section id="setup" className={styles.setup} aria-labelledby="setup-title"><div className={styles.container}><Heading eyebrow={setup.eyebrow} title={setup.title} description={setup.description} id="setup-title"/><div className={styles.setupGrid}>{setup.steps.map((step,index)=><article key={step.number}><b className={styles.iconBubble} aria-hidden="true">{setupIcons[index]}</b><span>{step.number}</span><h3>{step.title}</h3><p>{step.description}</p><a href={step.action.href} target="_blank" rel="noreferrer">설정 안내 ↗</a></article>)}</div></div></section>
