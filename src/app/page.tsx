  ...
  return <div id="top" className={styles.page}>
    ...
    <main id="main-content" tabIndex={-1}>
      <section className={`${styles.hero} ${styles.container}`} aria-labelledby="hero-title">
        ...
        <div className={styles.heroCopy}><p className={styles.eyebrow}>{hero.eyebrow}</p><h1 id="hero-title">{hero.title}</h1><p className={styles.lead}>{hero.description}</p><ul className={styles.heroFacts} aria-label="2기 교육 핵심 정보">{hero.facts.map((fact)=><li key={fact}>{fact}</li>)}</ul><div className={styles.actions}><Link className={styles.primaryAction} href={hero.primaryAction.href}>{hero.primaryAction.label}<span>→</span></Link><Link className={styles.secondaryAction} href={hero.secondaryAction.href}>{hero.secondaryAction.label}</Link></div><Link className={styles.achievement} href="/course#progress"><span aria-hidden="true">✓</span>{hero.achievement}<span aria-hidden="true">→</span></Link><small className={styles.heroNote}>✓ {hero.note}</small></div>
      </section>
