...
try {
  ...
  for (const copy of ["처음 한 번만,", "앱에서 무엇을", "ChatGPT Work", "질문은 Chat, 완성 문서는 Work", "데스크톱 앱에서는 이렇게 진행하세요.", "Google 계정 만들기 · Chrome 통일", "계정과 브라우저를 섞지 마세요.", "Windows 기본 브라우저를 Chrome으로 설정", "Edge에서 다른 계정으로 인증하지 않기", "ChatGPT 가입", "Codex", "GitHub", "Vercel", "Supabase 가입 · 계정 확인", "Sign in with GitHub", "Supabase 가입·대시보드 열기", "Supabase 프로젝트 연결 — 필요할 때만"]) {
  ...
  for (const copy of ["무엇을 할지 고르면", "총 54개", "대화 · 기획", "조사 · 학습", "이미지 · 디자인", "동영상 · 아바타", "코딩 · 웹 제작", "저장 · 배포 · 데이터", "처음이라면 이 세 개만", "ChatGPT", "NotebookLM", "Canva AI", "Runway", "HeyGen", "Synthesia", "CapCut", "Codex"]) assert(aiLinksHtml.includes(copy), `AI 링크 허브 누락: ${copy}`);
  assert((aiLinksHtml.match(/공식 사이트 새 창으로 열기/g) ?? []).length >= 54, "운영 AI 링크 허브의 공식 링크가 54개가 아닙니다.");
  ...
  for (const copy of ["디자인·코딩 참고자료", "AI·홈페이지 학습", "수업용 크롬 확장 프로그램", "웹 개발 문서", "아이콘 참고 사이트", "AI Together 참고자료 전체보기"]) assert(resourcesHtml.includes(copy), `무료 자료 페이지 AI Together 구성 누락: ${copy}`);
  assert(resourcesHtml.includes("https://ai-together-mu.vercel.app/resources"), "AI Together 자료실 연결이 누락됐습니다.");
  console.log("✓ 무료 자료 페이지 안에 AI Together 참고자료 구성");
