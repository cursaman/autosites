...
try {
  ...
  for (const copy of ["코딩 없이", "말 한마디에서", "이런 분께 추천합니다", "무엇을 만들 수 있나요?", "4주 동안 하나의", "계속 미루면 홈페이지는", "다음 기수 일정은 아직 미정", "당근에서 남은 자리 문의하기", "처음 시작하는 사람을 위한 실전 클래스"]) {
    assert(html.includes(copy), `콘텐츠 요청 결과 누락: ${copy}`);
  }
  console.log("✓ 요청 1 — 메인 콘텐츠와 작업 흐름");

  for (const copy of ["4주 후에는", "당근에서 남은 자리 문의하기", "80,000원"]) {
  ...
  for (const copy of ["4주 뒤, 직접 만든", "당근에서 남은 자리 문의하기", "지금 할 일", "4주 과정 문의", "남은 자리 안내받기", "4주 뒤 내 손에 남는 것", "내 주제의 반응형 홈페이지", "실제로 열리는 운영 URL", "수업에서 배우는 제작 방식", "바이브코딩 흐름 안내", "채팅 요청부터 실제 배포까지", "AutoSites 운영자", "2시간 수업표", "GitHub 저장소 생성·첫 Push", "수업이 끝날 때마다 기록합니다.", "첫 커밋과 Push 실습", "1기의 과정이", "MOVIEBOX", "1기 수강생 작품 · 운영 중", "수강생 작품 보기", "2기 사전 관심 접수", "공개 자료 준비 중", "동의하지 않아도 불이익이 없습니다.", "1기 교육이 이번 주 마무리됩니다.", "1기 4주차 진행 예정", "75%", "80,000원", "별도의 장소 이용료 없이", "오후 6시 정시 시작", "건물 주차 가능", "커피긱스 2층 8인룸", "자리 확인 후 신청 확정", "이 페이지에서는 이름·전화번호·이메일을 수집하거나 공개하지 않습니다."]) {
